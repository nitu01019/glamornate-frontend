...
const navItems: NavItem[] = [
  ...
  { href: '/spa/services', label: 'Services', icon: Scissors },
...
export default function SpaBottomNav() {
  ...
  const isActive = (href: string) => {
    if (href === '/spa/dashboard') {
      return pathname === '/spa/dashboard' || pathname === '/spa';
    }
    return pathname.startsWith(href);
  };

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-xl border-t border-amber-100/50 safe-area-bottom">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {active && (
                <span className="absolute top-1 w-1 h-1 bg-amber-500 rounded-full" />
              )}
            </Link>
          );
        })}
