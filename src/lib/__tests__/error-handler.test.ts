...
describe('getUserFriendlyMessage', () => {
  ...
  it('should return the mapped message for TIMEOUT', () => {
    const err = new TimeoutError();
    expect(getUserFriendlyMessage(err)).toBe('The request took too long. Please try again.');
  });

  it('should return the mapped message for VALIDATION_ERROR', () => {
    const err = new ValidationError();
    expect(getUserFriendlyMessage(err)).toBe('Please check your input and try again.');
  });

  // 2026-05-11 (Cinder-D16 / F43): unmapped codes used to fall back to the
  // raw error message, leaking Firebase internal strings to the user.
  // Now they fall straight to the generic copy. Sentry still captures the
  // raw message at upstream catch sites — no diagnostic loss.
  it('should fall back to generic copy for unmapped codes (no raw-message leak)', () => {
