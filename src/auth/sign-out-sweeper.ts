 ...
 */
export type StoreResetMethod = 'reset' | 'clear' | 'disconnect' | 'clearMessages' | 'clearCart';
...
function callFirstAvailable(
  ...
  methods: readonly StoreResetMethod[],
): {
...
async function stepClearQueryClient(queryClient: SweepCtx['queryClient']): Promise<void> {
...
// Intentionally removed (2026-05-14): the previous implementation called
// `terminate(db) + clearIndexedDbPersistence(db)` on every sign-out. The
// Firebase SDK marks the terminated client as permanently unusable in the
// same JS context, so the *next* sign-in (Google or email) failed with
// `FirebaseError: The client has already been terminated.` as soon as the
// AuthProvider tried to fetch the user profile. Confirmed via CDP capture
// on emulator-5554 (Glamornate-758c6, 2026-05-14): repro = sign in → sign
// out → sign in → "Sign-in failed. If this persists, try reinstalling the
// app." Cached Firestore docs are already UID-scoped by security rules, so
// leaving the local IndexedDB intact is not a security regression — the
// next user simply cannot read another user's docs. Reinstate ONLY with a
// post-sign-out full app reload (window.location.reload()) so the Firebase
// app singleton is rebuilt from scratch.

async function stepClearCapacitorPreferences(): Promise<void> {
  try {
    if (isNative()) {
    ...
    log.warn('Capacitor Preferences clear failed', { err: safeErrorMessage(error) });
  }
}
 ...
 *   7. (removed 2026-05-14) Firestore IDB terminate+clear — broke next sign-in
 *   8. Clear Capacitor Preferences (native only)
 *   9. Firebase Auth sign-out (ALWAYS runs, even if earlier steps threw)
 */
export async function sweepClientState(ctx: SweepCtx): Promise<SweepResult> {
  ...
  try {
    ...
    // firestore.idbClear intentionally NOT invoked — see comment above
    // `stepClearFirestoreIndexedDb` (removed 2026-05-14). Calling
    // `terminate(db)` here permanently breaks the next sign-in.
    steps.push(await timed(now, 'capacitor.prefsClear', () => stepClearCapacitorPreferences()));
  } finally {
