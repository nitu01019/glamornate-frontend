...
import type { ApiResponse, HttpMethod } from '@/shared/contracts';
...
/**
 * Token-revocation callback. Phase 9 (Auth Bridge Fix, 2026-05-08): when the
 * backend rejects with envelope `code: 'token-revoked'` or `'invalid-token'`,
 * forcing `getIdToken(true)` does not help — the session is dead. The auth
 * provider registers a callback here so the client can trigger a full
 * sign-out + redirect.
 */
export type TokenRevokedHandler = () => void;

let onTokenRevoked: TokenRevokedHandler | null = null;

/**
 * Register a handler invoked when the backend signals token-revoked /
 * invalid-token. Pass `null` to unregister (used by the auth provider's
 * effect cleanup).
 */
export function setOnTokenRevoked(handler: TokenRevokedHandler | null): void {
...
async function buildHeaders(
  ...
  if (!skipAuth && idTokenProvider) {
    try {
      ...
      // M-F1 (2026-05-11): if a forced refresh failed with a terminal
      // Firebase code (the session is dead — token-expired without a valid
      // refresh, user-disabled, invalid-user-token, refresh-token network
      // failure that can't be retried), drop the Authorization header AND
      // invoke the registered revoke handler so the auth-provider can sweep
      // state and surface a "sign in again" toast. Without this, requests
      // proceed unauth and downstream consumers see opaque 401s with no
      // recovery — user gets stuck in a 401-storm with a stale session.
      ...
      if (isTerminal && onTokenRevoked) {
        try {
          ...
          apiLogger.error('onTokenRevoked handler threw', handlerError);
        }
      }
    }
  }
...
async function performRequest<T>(
  ...
  try {
    // 2026-05-11 (Forge-D1 / F16): drop `credentials: 'include'`. The auth
    // model is Bearer-token-only (attached via setIdTokenProvider above);
    // no cookies are consumed by the BE. Sending credentials cross-origin
    // requires the BE to echo Access-Control-Allow-Credentials: true with
    // a specific (non-wildcard) Access-Control-Allow-Origin or the browser
    // blocks the response — a CORS footgun for no benefit. Default
    // `same-origin` is sufficient.
  ...
  // 401 + token-revoked / invalid-token → do NOT retry; force full sign-out
  // via the auth-provider-registered handler.
  if (response.status === 401 && attempt === 0) {
    ...
    const REVOKED: AuthErrorCodeT = 'token-revoked';
    const INVALID: AuthErrorCodeT = 'invalid-token';
    if (envelope.code === REVOKED || envelope.code === INVALID) {
      apiLogger.warn(`Received ${envelope.code}; forcing full re-authentication.`, { path });
      if (onTokenRevoked) {
        try {
          onTokenRevoked();
        } catch (handlerError) {
  ...
  if (!response.ok) {
    // Rate-limit (429): backend emits envelope `code: 'rate-limited'` plus a
    // `Retry-After` header. Surface a typed RateLimitError so React Query
    // handlers and `getUserFriendlyMessage` can render an actionable copy.
    const errEnvelope = envelopeFrom(parsed);
    if (response.status === 429 && errEnvelope.code === 'rate-limited') {
      const retryHeader = response.headers.get('Retry-After');
      const parsedRetry = retryHeader != null ? Number(retryHeader) : NaN;
      const retryAfterSeconds =
        Number.isFinite(parsedRetry) && parsedRetry > 0 ? parsedRetry : undefined;
      throw new RateLimitError(
        errEnvelope.error ?? 'Too many requests. Please try again shortly.',
        {
          retryAfterSeconds,
          context: { path, requestId: errEnvelope.requestId },
        },
      );
    }
...
export { ApiError, ApiTimeoutError, isApiError } from '@/lib/api-errors';
