#!/usr/bin/env bash
# Refresh src/data/catalog/ from a sibling private monorepo.
...
DEST="$(cd "$(dirname "$0")/.." && pwd)/src/data/catalog/"
...
cp "$DEST/VENDORED.md" /tmp/.vendored-md.data-catalog
rm -rf "$DEST"
mkdir -p "$DEST"
rsync -a --exclude='*.test.ts' --exclude='__tests__' "$SRC" "$DEST"
mv /tmp/.vendored-md.data-catalog "$DEST/VENDORED.md"
